import os
import pandas as pd
import numpy as np
import cv2
from math import sqrt

def run_analysis(image_folder, fps=10000, mm_per_pixel=0.00128):
    """
    Process a sequence of images to calculate football dynamics metrics and annotate images.
    
    Args:
        image_folder (str): Path to the directory containing .bmp image files.
        fps (float): Frame rate of the image sequence (frames per second). Default: 10000 fps.
        mm_per_pixel (float): Calibration factor (mm per pixel). Default: 0.00128 (172 pixels ≈ 0.22 m).
    
    Returns:
        dict: A dictionary containing the calculated metrics:
              - inbound_velocity (m/s)
              - outbound_velocity (m/s)
              - cor (coefficient of restitution)
              - contact_time (s)
              - deformation (mm)
    """
    # Load image files
    image_files = sorted([os.path.join(image_folder, f) for f in os.listdir(image_folder) if f.endswith('.bmp')])
    if not image_files:
        raise ValueError(f"No .bmp files found in {image_folder}.")

    # Extract frame numbers from filenames (e.g., "frame_0000.bmp" -> 0)
    frame_numbers = []
    for img_file in image_files:
        filename = os.path.basename(img_file)
        try:
            frame_num = int(filename.split('_')[1].replace('.bmp', ''))
            frame_numbers.append(frame_num)
        except (IndexError, ValueError):
            frame_numbers.append(0)  # Fallback if filename format is unexpected

    # Read the first image to initialize
    frame = cv2.imread(image_files[0], cv2.IMREAD_GRAYSCALE)
    if frame is None:
        raise ValueError(f"Failed to load the first image: {image_files[0]}")

    # Fix surface_y to 446 (where the green line is located)
    surface_y = 446
    print(f"Surface fixed at y = {surface_y}")

    # Kalman Filter Setup for Stable Circle Detection
    kalman = cv2.KalmanFilter(4, 2)
    kalman.measurementMatrix = np.array([[1, 0, 0, 0], [0, 1, 0, 0]], np.float32)
    kalman.transitionMatrix = np.array([[1, 0, 1, 0], [0, 1, 0, 1], [0, 0, 1, 0], [0, 0, 0, 1]], np.float32)
    kalman.processNoiseCov = np.eye(4, dtype=np.float32) * 0.03
    kalman.statePre = np.array([[frame.shape[1] // 2], [frame.shape[0] // 2], [0], [0]], np.float32)

    # Initialize variables
    D_original = None
    last_valid_radius = None
    last_valid_contact_points = None
    data_records = []
    dot_records = []
    ball_positions = []  # To track ball center (cy) over time
    contact_frames = []  # To track frames where ball is in contact

    # Process frames
    for i, (img_file, frame_num) in enumerate(zip(image_files, frame_numbers)):
        frame = cv2.imread(img_file, cv2.IMREAD_GRAYSCALE)
        if frame is None:
            print(f"Failed to load frame {frame_num}: {img_file}")
            continue

        # Convert to color for annotations
        frame_color = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)

        blurred = cv2.GaussianBlur(frame, (5, 5), 0)
        thresh = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2)

        # Adjust HoughCircles parameters for better detection
        circles = cv2.HoughCircles(frame, cv2.HOUGH_GRADIENT, dp=1.2, minDist=50, param1=50, param2=20, minRadius=10, maxRadius=150)

        prediction = kalman.predict()
        predicted_cx, predicted_cy = int(prediction[0]), int(prediction[1])

        cx, cy, radius = None, None, None
        if circles is not None:
            circles = np.uint16(np.around(circles))
            for circle in circles[0, :]:
                cx, cy, radius = circle
                print(f"Frame {frame_num}: Circle detected at ({cx}, {cy}) with radius {radius}")
                break

            if last_valid_radius is not None:
                radius = int(0.85 * last_valid_radius + 0.15 * radius)
            last_valid_radius = radius
            kalman.correct(np.array([[np.float32(cx)], [np.float32(cy)]]))
        else:
            print(f"Frame {frame_num}: No circle detected, using predicted position ({predicted_cx}, {predicted_cy})")
            cx, cy = predicted_cx, predicted_cy
            radius = last_valid_radius if last_valid_radius is not None else 50

        ball_positions.append((frame_num, cy))  # Track frame number and y-position

        top_dot_x, top_dot_y = None, None
        bottom_dot_x, bottom_dot_y = None, None
        contact_dot1_x, contact_dot1_y = None, None
        contact_dot2_x, contact_dot2_y = None, None
        diameter = 0
        contact_length = 0

        if radius is not None:
            top_point = (cx, cy - int(radius))
            bottom_point = (cx, cy + int(radius))

            # Relaxed contact detection
            contact_threshold = 5  # Allow a small margin for contact detection
            if abs(bottom_point[1] - surface_y) <= contact_threshold:
                bottom_point = (cx, surface_y)

            top_dot_x, top_dot_y = top_point
            bottom_dot_x, bottom_dot_y = bottom_point

            diameter = bottom_point[1] - top_point[1]
            diameter = max(diameter, 20)

            if D_original is None and bottom_point[1] < surface_y - 50:
                D_original = diameter
                print(f"Frame {frame_num}: D_original set to {diameter} pixels")

            cv2.circle(frame_color, (cx, cy), int(radius), (255, 0, 0), 1)

            contact_point1 = None
            contact_point2 = None
            if abs(bottom_point[1] - surface_y) <= contact_threshold:
                delta_y = surface_y - cy
                discriminant = radius**2 - delta_y**2
                if discriminant >= 0:
                    sqrt_disc = np.sqrt(discriminant)
                    x1 = int(cx - sqrt_disc)
                    x2 = int(cx + sqrt_disc)
                    contact_point1 = (x1, surface_y)
                    contact_point2 = (x2, surface_y)
                    contact_length = x2 - x1
                    contact_frames.append(frame_num)
                    print(f"Frame {frame_num}: Contact detected, length = {contact_length} pixels")

                    if last_valid_contact_points is not None:
                        x1_prev, x2_prev = last_valid_contact_points
                        if abs(x1 - x1_prev) > 10 or abs(x2 - x2_prev) > 10:
                            x1, x2 = x1_prev, x2_prev
                    
                    last_valid_contact_points = (x1, x2)

                    contact_dot1_x, contact_dot1_y = contact_point1
                    contact_dot2_x, contact_dot2_y = contact_point2

                    cv2.circle(frame_color, contact_point1, 3, (0, 255, 255), -1)
                    cv2.circle(frame_color, contact_point2, 3, (0, 255, 255), -1)

        cv2.line(frame_color, (0, surface_y), (frame.shape[1], surface_y), (0, 255, 0), 1)

        if top_point is not None and bottom_point is not None:
            cv2.circle(frame_color, top_point, 3, (255, 255, 0), -1)
            cv2.circle(frame_color, bottom_point, 3, (255, 255, 0), -1)

        annotated_path = os.path.join(image_folder, f"annotated_frame_{frame_num:04d}.bmp")
        cv2.imwrite(annotated_path, frame_color)

        data_records.append([frame_num, cx, cy, diameter, contact_length])
        dot_records.append([frame_num, top_dot_x, top_dot_y, bottom_dot_x, bottom_dot_y, 
                           contact_dot1_x, contact_dot1_y, contact_dot2_x, contact_dot2_y])

    # Create DataFrames
    df = pd.DataFrame(data_records, columns=["Frame", "Ball_X", "Ball_Y", "Diameter", "Contact_Length"])
    dot_df = pd.DataFrame(dot_records, columns=["Frame", 
                                                "Top_Dot_X", "Top_Dot_Y", 
                                                "Bottom_Dot_X", "Bottom_Dot_Y", 
                                                "Contact_Dot1_X", "Contact_Dot1_Y", 
                                                "Contact_Dot2_X", "Contact_Dot2_Y"])

    # Calculate Metrics
    # Default values in case of failure
    inbound_velocity = 0.0
    outbound_velocity = 0.0
    cor = 0.0
    contact_time = 0.0
    deformation = 0.0

    try:
        if not contact_frames:
            print("Warning: No contact frames detected. Using default values.")
        else:
            # Calculate time column
            df["Time_s"] = (df["Frame"] - df["Frame"].min()) / fps
            time_interval = 1 / fps

            # Determine contact start and end
            contact_start = min(contact_frames)
            contact_end = max(contact_frames)

            # Inbound velocity: Use the two frames just before contact_start
            pre_contact_frames = df[df["Frame"] < contact_start]["Frame"].sort_values()
            if len(pre_contact_frames) >= 2:
                frame_2 = pre_contact_frames.iloc[-1]  # Last frame before contact
                frame_1 = pre_contact_frames.iloc[-2]  # Second-to-last frame before contact
                y_1 = df.loc[df["Frame"] == frame_1, "Ball_Y"].values[0]
                y_2 = df.loc[df["Frame"] == frame_2, "Ball_Y"].values[0]
                vi_pixels = (y_2 - y_1) / time_interval
                inbound_velocity = vi_pixels * mm_per_pixel  # m/s
                print(f"Inbound velocity calculated using frames {frame_1} and {frame_2}: {inbound_velocity:.2f} m/s")
            else:
                print("Warning: Not enough frames before contact to calculate inbound velocity.")

            # Outbound velocity: Use the two frames just after contact_end
            post_contact_frames = df[df["Frame"] > contact_end]["Frame"].sort_values()
            if len(post_contact_frames) >= 2:
                frame_1 = post_contact_frames.iloc[0]  # First frame after contact
                frame_2 = post_contact_frames.iloc[1]  # Second frame after contact
                y_1 = df.loc[df["Frame"] == frame_1, "Ball_Y"].values[0]
                y_2 = df.loc[df["Frame"] == frame_2, "Ball_Y"].values[0]
                vo_pixels = (y_2 - y_1) / time_interval
                outbound_velocity = vo_pixels * mm_per_pixel  # m/s
                print(f"Outbound velocity calculated using frames {frame_1} and {frame_2}: {outbound_velocity:.2f} m/s")
            else:
                print("Warning: Not enough frames after contact to calculate outbound velocity.")

            # COR calculation
            if inbound_velocity != 0:
                cor = abs(outbound_velocity) / abs(inbound_velocity)
                print(f"COR calculated: {cor:.2f}")
            else:
                print("Warning: Inbound velocity is 0, cannot calculate COR.")

            # Contact time
            contact_time = (contact_end - contact_start + 1) / fps  # in seconds
            print(f"Contact frames: {contact_start} to {contact_end}, Contact time: {contact_time:.4f} s")

            # Deformation
            max_diameter = df["Diameter"].max()
            deformation = (max_diameter - D_original) * mm_per_pixel if D_original else 0  # in mm
            print(f"Deformation: {deformation:.2f} mm")

    except Exception as e:
        print(f"Error calculating metrics: {str(e)}")
        # Return default values if calculation fails

    return {
        "inbound_velocity": inbound_velocity,  # Already in m/s
        "outbound_velocity": outbound_velocity,  # Already in m/s
        "cor": cor,
        "contact_time": contact_time,
        "deformation": deformation
    }
